...
class SnapmeterAccountDataSource(ModelMixin, Base):
    ...
    meter_data_sources = relationship("SnapmeterMeterDataSource", backref=backref("account_ds"))  # FIXME

    @classmethod
    def by_hex_id(cls, hex_id):
        return db.session.query(SnapmeterAccountDataSource).filter_by(hex_id=hex_id).first()

    @classmethod
    def by_account_and_type(cls, account_oid, account_type, username=None):
        query = db.session.query(SnapmeterAccountDataSource). \
            filter_by(account=account_oid, source_account_type=account_type)
        if username:
...
class SnapmeterMeterDataSource(ModelMixin, Base):
    ...
    def by_meter_and_type(cls, meter_id, source_type):
        return db.session.query(SnapmeterMeterDataSource). \
    ...
    def to_json(self, account_id=None, include_credentials=False, str_ids=False):
        ...
        if include_credentials and self.account_data_source:
            ...
            rval["accountId"] = str(self.account_ds.account) if str_ids \
                else self.account_ds.account
        return rval
