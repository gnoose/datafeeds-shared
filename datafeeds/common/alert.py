...
from datafeeds.models import SnapmeterAccountDataSource, SnapmeterAccountMeter, SnapmeterMeterDataSource
...
def disable_logins(acct_ds: SnapmeterAccountDataSource):
    ...
    query = db.session.query(SnapmeterAccountMeter).\
    ...
    meter_list = []
    for sam in query:
        meter_list.append("\t%s %s (%s)\n" % (sam.account_obj.name, sam.meter_obj.name, sam.meter))
    if not meter_list:
        return
    msg = "Login failed for %s; disabled scrapers for meters:\n%s" % (
        acct_ds.name, "\n".join(meter_list))
