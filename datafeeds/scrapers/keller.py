...
KellerIdentifier = namedtuple("KellerIdentifier", ["route_number", "service_address_id", "resident_number"])
...
class BillHistoryPage:
    ...
    def gather_data(self, user_id: KellerIdentifier, start: date, end: date) -> List[Optional[bytes]]:
        pdf_link_xpath = "//img[@title='Click to view bill.  You must have Adobe reader installed to view your bill.']"
        WebDriverWait(self.driver, 60).until(ec.presence_of_element_located((By.XPATH, pdf_link_xpath)))
...
class HomePage:
    ...
    def to_bill_history(self):
        elt = WebDriverWait(self.driver, 60).until(ec.presence_of_element_located((By.ID, 'ACTION_TO_PERFORM')))
        select = Select(elt)
...
class LoginPage:
    ...
    def login(self, user_id: KellerIdentifier, password: str) -> HomePage:
        ...
        WebDriverWait(self.driver, 5) \
            .until(ec.presence_of_element_located((By.ID, "ROUTE_NUMBER"))) \
            .send_keys(user_id.route_number)

        self.driver.find_element_by_id("SERVICE_ADDRESS_ID").send_keys(user_id.service_address_id)
        self.driver.find_element_by_id("RESIDENT_NUMBER").send_keys(user_id.resident_number)
        self.driver.find_element_by_xpath("//tr/td//input[@id='PASSWORD']").send_keys(password)
        self.driver.find_element_by_xpath("//input[@value='Continue']").click()

        try:
            WebDriverWait(self.driver, 5) \
                .until(ec.presence_of_element_located((By.XPATH, "//center/div[@class='ErrorMsg']")))
...
class KellerScraper(BaseWebScraper):
    ...
    def keller_id(self) -> KellerIdentifier:
        ...
        if len(elements) != 3 or len(elements[0]) != 3 or len(elements[1]) != 7 or len(elements[2]) != 3:
            raise KellerScraperException("Keller service account numbers must be of the form \"XXX-XXXXXXX-XXX\".")
    ...
    def _execute(self):
        ...
        bills = bill_history_page.gather_data(self.keller_id, self.start_date, self.end_date)

        _log("Acquired %d bills (%s bytes total)." % (len(bills), sum(len(b) for b in bills)))
